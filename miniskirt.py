...
def get_temperature(location: str,
    ...
    weather_data = {}
    if response.ok:
        if 'error' not in list(response.json().keys()):
            data = response.json()
            weather_data['description'] = ' and '.join(
                [i['description'] for i in data['weather']])
            weather_data['feels_like'] = f"{data['main']['feels_like']} {units_dict[units]}"
        else:
            console.log(
                f'[bold red]Weather API call failed - {response.json()["error"]} status code')
    else:
        console.log(
            f'[bold red]Weather API call failed - {response.status_code} status code')
...
def miniskirt(
    ...
    units: str = typer.Option(
        "metric",
        help="Unit of temperature measurement. Defaults to 'metric'.")):
    ...
    if feels_like_temp > mini_thresh:
        console.print(
            f"{weather_report_str}. Get in a mini! :dancers:",
            style="bold red")
    elif feels_like_temp < mini_thresh:
        console.print(
            f"{weather_report_str}. I reckon it's too cold for one :tired_face:",
            style="bold blue")
